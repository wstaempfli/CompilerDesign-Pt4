...
%type <Ast.stmt Ast.node> stmt_exp
...
stmt: 
  ...
  | FOR LPAREN init=separated_list(COMMA, vdecl) SEMI cond=option(exp) SEMI update=option(stmt_exp) RPAREN b=block
    { loc $startpos $endpos @@ For(init, cond, update, b) }
